...
This document outlines the architecture, contribution workflow, local code generation instructions, and quality standards required for maintaining and extending the SDK.
...
- **Policy:** **READ-ONLY**. Under no circumstances should files inside the `openapi/` directory be edited manually. Any manual changes will be overwritten during the next code generation cycle.
...
- **Purpose:** Provides a clean, ergonomic, and idiomatic Go interface for developers:
  ...
  - [`enrichment.go`](file:///Users/hadi/dev/start-ups/xyo/sdks/golang/enrichment.go): Exposes the [`Enrichment`](file:///Users/hadi/dev/start-ups/xyo/sdks/golang/enrichment.go) interface with methods like `EnrichTransaction`, `EnrichTransactions`, `GetEnrichmentStatus`, and `DownloadEnrichmentCollection` (which handles downloading and decompressing `tar.gz` batch archives), with full `context.Context` cancellation support.
...
| :--- | :--- | :--- |
| **API Endpoints & Contracts**<br>(New endpoints, URL changes, HTTP methods) | [`xyo-financial/specs`](https://github.com/xyo-financial/specs) | Submit PR to update `openapi.yml`. Once merged, regenerate the Go `openapi/` layer. |
...
## 4. Local Code Generation

When the OpenAPI specification in `xyo-financial/specs` (`../specs/openapi.yml`) is updated, the low-level `openapi/` package must be regenerated.
...
Run the following command from the root of the Go SDK repository (`/Users/hadi/dev/start-ups/xyo/sdks/golang`):
...
Ensure the entire module compiles cleanly:
```bash
go build ./...
```

### 2. Static Analysis & Vetting
Run standard Go vetting to detect suspicious constructs:
```bash
go vet ./...
```

### 3. Test Suite & Race Detector
...
### 4. Code Formatting
Format all files according to standard Go conventions:
```bash
gofmt -s -w .
```
...
### 6. Containerized Build (Optional / CI Parity)
...
2. Create and push an annotated Git tag:
   ...
   ```
3. The automated CI/CD release pipeline ([`.github/workflows/release.yml`](file:///Users/hadi/dev/start-ups/xyo/sdks/golang/.github/workflows/release.yml)) will:
